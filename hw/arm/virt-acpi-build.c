...
static void nvidia_get_bridge_window(PCIBus *bus, void *opaque)
{
    ...
    if (!bridge) {
        ...
        ncfg->wlimit = pci_bridge_get_limit(bridge,
                                            PCI_BASE_ADDRESS_MEM_TYPE_32);
        ncfg->wbase64 = pci_bridge_get_base(bridge,
                                            PCI_BASE_ADDRESS_MEM_PREFETCH);
        ncfg->wlimit64 = pci_bridge_get_limit(bridge,
                                              PCI_BASE_ADDRESS_MEM_PREFETCH);
    }
}

static void nvidia_update_bridge_window(PCIBus *bus, uint64_t base,
                                        uint64_t limit)
{
    PCIDevice *bridge = pci_bridge_get_device(bus);
...
static void fix_pci_bar_GB200_nvidia(PCIDevice *dev, PhysBAR *pbars)
{
    ...
    for (idx = PCI_ROM_SLOT - 1 ; idx >= 0; idx--) {
        ...
        pbar[idx].addr = QEMU_ALIGN_UP(pbars[idx].addr,
                                       dev->io_regions[idx].size);
        pbar[idx].end = pbar[idx].addr + dev->io_regions[idx].size - 1;
    }

    /*
     * Make sure BAR1 gets GPA=HPA, adjust other two BARs
     * accordingly to avoid region conflict.
     */
    overlap = true;
    while (overlap) {
        overlap = false;
        ...
        for (idx = 0; idx <=  PCI_ROM_SLOT - 1; idx++) {
            ...
            for (int j = 0; j < PCI_ROM_SLOT; j++) {
                ...
                if (ranges_overlap(pbar[idx].addr, dev->io_regions[idx].size,
                                   pbar[j].addr, dev->io_regions[j].size)) {
                    pbar[j].addr = QEMU_ALIGN_UP(pbar[idx].addr +
                                                 dev->io_regions[idx].size,
                                                 dev->io_regions[j].size);
                    overlap = true;
                }
            }
        }
...
static void nvidia_dev_vfio(PCIBus *bus, PCIDevice *dev, void *opaque)
{
    ...
    for (idx = 0, pbar = pbars; idx < PCI_ROM_SLOT; idx++, pbar++) {
        ...
        if (pbar->flags & IORESOURCE_MEM_64) {
...
static void nvidia_dev_unassigned_mmio64(PCIBus *bus, PCIDevice *dev,
                                         void *opaque)
{
