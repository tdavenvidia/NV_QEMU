...
static int tegra241_cmdqv_init_vcmdq_page0(Tegra241CMDQV *s)
{
    ...
    s->vcmdq_page0 = smmu_viommu_get_shared_page(ARM_SMMU(s->smmu_dev),
                                                VCMDQ_REG_PAGE_SIZE);
    if (!s->vcmdq_page0) {
...
static int tegra241_cmdqv_setup_vcmdq(Tegra241CMDQV *s, int index)
{
    ...
    vqueue = iommufd_viommu_alloc_queue(s->viommu,
...
static void cmdqv_unrealize(DeviceState *d)
{
    ...
    smmu_viommu_put_shared_page(ARM_SMMU(s->smmu_dev), s->vcmdq_page0,
                               VCMDQ_REG_PAGE_SIZE);
}
