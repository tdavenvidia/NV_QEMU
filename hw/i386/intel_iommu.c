...
static inline int vtd_dev_get_rid2pasid(IntelIOMMUState *s, uint8_t bus_num,
                                  ...
                                  Error **errp)
{
    ...
    trace_vtd_device_attach_hwpt(idev->devid, rid_pasid, hwpt->hwpt_id, ret);
    if (ret) {
        if (vtd_pe_pgtt_is_flt(pe)) {
                                       ...
                                       Error **errp)
{
    ...
    QLIST_FOREACH(s2_hwpt, &container->s2_hwpt_list, next) {
        ret = vtd_device_attach_hwpt(vtd_hiod, rid_pasid, pe,
                                     s2_hwpt, hwpt, &err);
        if (ret) {
    ...
    ret = vtd_device_attach_hwpt(vtd_hiod, rid_pasid, pe, s2_hwpt, hwpt, errp);
    if (ret) {
        goto err_attach_hwpt;
    }

found_hwpt:
    trace_vtd_device_attach_container(iommufd->fd, idev->devid, rid_pasid,
                                        ...
                                        Error **errp)
{
    ...
    trace_vtd_device_detach_container(iommufd->fd, idev->devid, rid_pasid);
    vtd_device_detach_hwpt(vtd_hiod, rid_pasid, pe, hwpt, errp);
    vtd_ioas_container_put_s2_hwpt(s2_hwpt);
}
                                     ...
                                     Error **errp)
{
    ...
    QLIST_FOREACH(container, &s->containers, next) {
        ...
        if (vtd_device_attach_container(vtd_hiod, container,
...
err_listener_register:
    vtd_device_detach_container(vtd_hiod, rid_pasid, pe, hwpt, errp);
                                   ...
                                   uint32_t rid_pasid)
{
    ...
    return vtd_device_attach_iommufd(vtd_hiod, rid_pasid, pe,
                                  ...
                                  uint32_t rid_pasid)
{
    ...
    vtd_device_detach_iommufd(vtd_hiod, rid_pasid, cached_pe,
                              &vtd_pasid_as->hwpt, &error_abort);
                                ...
                                VTDPASIDEntry *pe, VTDPASIDOp op)
{
    ...
    case VTD_PASID_BIND:
    {
        ret = vtd_device_attach_pgtbl(vtd_hiod, pe, vtd_pasid_as, rid_pasid);
        break;
    }
    case VTD_PASID_UNBIND:
    {
        ret = vtd_device_detach_pgtbl(vtd_hiod, vtd_pasid_as, rid_pasid);
        break;
    }
    default:
...
static void vtd_flush_pasid_iotlb_all(IntelIOMMUState *s, uint16_t domain_id,
                                      uint32_t pasid, hwaddr addr,
                                            ...
                                            VTDPASIDEntry *pe)
{
    ...
    return vtd_ce_get_rid2pasid_entry(s, &ce, pe, pasid);
}
                                ...
                                gpointer user_data)
{
     ...
     *   should invalidate QEMU piotlb together with this change.
     */
    return false;
remove:
    /*
                                                   ...
                                                   uint32_t pasid)
{
    ...
    if (!vtd_pasid_as) {
         ...
         * binding and also serves as pasid-cache management entry.
         *
                                 ...
                                 VTDPASIDCacheInfo *pc_info)
{
    ...
    vtd_iommu_lock(s);
    /* Step 1: loop all the existing vtd_pasid_as instances */
